...
<?php
  ...
  $sql_movie = "SELECT * FROM movie WHERE Title = '{$_GET['movie_Title']}'";
  $row_movie = $db->query($sql_movie)->fetch(PDO::FETCH_OBJ);
  $sql_act = "SELECT * FROM act WHERE movie_movie_ID = '$row_movie->movie_ID'";
  $row_act = $db->query($sql_act);
  $sql_direct = "SELECT * FROM direct WHERE movie_movie_ID = $row_movie->movie_ID";
  $row_direct = $db->query($sql_direct)->fetch(PDO::FETCH_OBJ);
  $sql_eval = "SELECT * FROM evaluation WHERE movie_movie_ID = $row_movie->movie_ID";
          ...
          <div class="card-body">
            ...
            <p class="card-text">
                ...
                감독 : <a href="./search_dir.php?director_Name=<?php echo $row_direct->director_Name ?>" method="get">
                      <?php echo $row_direct->director_Name ?><br/>
                      </a>
                배우 : 
                <?php
